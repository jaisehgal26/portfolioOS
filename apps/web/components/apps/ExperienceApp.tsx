...
export function ExperienceApp() {
  ...
  return (
    ...
    >
      <ExperienceDetail key={activeId} roleId={activeId} />
    </AppTwoPane>
  );
}

/** A single role's detail. Reused by the Finder. */
export function ExperienceDetail({ roleId }: { roleId: string }) {
  const reduced = usePrefersReducedMotion();
  const role = experience.find((e) => e.id === roleId) ?? experience[0];
